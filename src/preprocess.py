...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='src/data/KQAPro/')
    parser.add_argument('--model_path',
                        type=str,
                        default='./vicuna_data/vicuna-7b-v1.1/')
    parser.add_argument('--output_dir', type=str, default='src/output/KQAPro/')
    args = parser.parse_args()

    print('Build kb vocabulary')
    vocab = {'answer_token_to_idx': {}}
    print('Load questions')
    train_set = json.load(open(os.path.join(args.data_path, 'train.json')))
    val_set = json.load(open(os.path.join(args.data_path, 'val.json')))
    test_set = json.load(open(os.path.join(args.data_path, 'test.json')))
