...
def hit_at_k(predictions: torch.Tensor,
    ...
    return torch.where(indices == ground_truth_idx, one_tensor,
                       zero_tensor).sum().item()
...
def mrr(predictions: torch.Tensor, ground_truth_idx: torch.Tensor) -> float:
    ...
    return (1.0 /
            (indices == ground_truth_idx).nonzero()[:, 1].float().add(1.0)
            ).sum().item()
